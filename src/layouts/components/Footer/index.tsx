...
import { useCreateNewsletter } from "@/queries/newslatter";
...
const Footer: React.FC = () => {
  ...
  const { mutateAsync: create, isLoading: newsletterCreating } =
    useCreateNewsletter();
  // On Submit Function
  const onSubmit = async (data: any) => {
    message.open({
      type: "loading",
      content: "Subscribing to GRADE BD..",
      duration: 0,
    });
    ...
    <footer>
      ...
      <AppBar className="bg-slate-800 relative shadow-none pt-16 z-20">
        {/* <Avatar
						...
						className="w-fit"
					/> */}
        <Grid container className="pb-0 px-8" rowGap={5} columnGap={3}>
          <Grid item xs={12} lg={3.5}>
            <Typography variant="subtitle2" className="font-semibold uppercase">
              {t("FOOTER.COMPANY")}
            </Typography>
            <Typography variant="body2" className="my-5 md:max-w-xs">
              {t("META.DESCRIPTION")}
            </Typography>
          </Grid>

          <Grid item xs={5.7} md={3} lg={1.2}>
            <Typography variant="subtitle2" className="font-semibold uppercase">
              {t("FOOTER.COMPANY_NAME")}
            </Typography>

            <div className="mt-3 flex flex-col gap-2">
              <Link href={"https://www.gradebd.com/about/"} target="_blank">
                <Typography
                  variant="caption"
          ...
          <Grid item xs={5.7} md={3} lg={1.9}>
            ...
            <div className="mt-3 flex flex-col gap-2">
              ...
              <Link href={`tel:${t("FOOTER.CONTACT_NUMBER")}`} target="_blank">
                <Typography variant="caption">
          ...
          <Grid item xs={5.7} md={5.3} lg={4}>
            ...
            <Typography variant="body2" className="my-2 md:max-w-xs">
              {t("FOOTER.NEWSLETTER_DESCRIPTION")}
            </Typography>
            <form
              onSubmit={(e) => {
                e.preventDefault(); // !important
                onSubmit({ email: newsletterEmail });
                // Submit from here
                setNewsletterEmail("");
              }}
            >
